...
(function(A, o) {
	...
	function fb(a) {
		try {
			...
			a.removeAttribute && a.removeAttribute(Z)
		}
	}
	function $(a, b, c) {
	...
	function ha(a, b, c) {
		if (b && b.indexOf(",") >= 0) m(b.split(","), function() {
			...
			var d = a[Z] || null;
			if (d) if (b === o) d in v && (m(v[d], function(b, c) {
				...
				if (J !== "") {
					...
					if (l === "font") {
						...
						m(w, function(a, b) {
							...
							a === "size" && (K["font-size"] = g[parseInt(b, 10) - 1] || "", delete w[a]);
	...
	function Ma(a, b, c) {
		...
		function g(e, l) {
			for (var j = e.firstChild, o; j;) {
				...
				if (m >= 0) return !1;
				o = j.nextSibling;
				if (n > 0) if (j.nodeType == 1) if (r >= 0 && p <= 0) b && l.appendChild(j.cloneNode(!0)), c && f.push(j);
				else {
					var q;
					b && (q = j.cloneNode(!1), l.appendChild(q));
					if (g(j, q) === !1) return !1
				} else if (j.nodeType == 3 && (j = j == k.startContainer ? d(j, k.startOffset, j.nodeValue.length) : j == k.endContainer ? d(j, 0, k.endOffset) : d(j, 0, j.nodeValue.length), b)) try {
					l.appendChild(j)
				} catch (s) {}
				j = o
			}
	...
	function qb(a, b) {
		...
		for (var r = 0, p = g.length; r < p; r++) {
			...
			if (j.nodeType == 1) {
				var l = a.duplicate(),
					m, o = f(j),
					q = j;
				o.isControl() && (m = c.createElement("span"), o.after(m), q = m, i += o.text().replace(/\r\n|\n|\r/g, "").length);
				ma(l, q);
		...
		i -= n.text.replace(/\r\n|\n|\r/g, "").length;
		if (k > 0 && h.nodeType == 3) for (c = h.previousSibling; c && c.nodeType == 3;) i -= c.nodeValue.length, c = c.previousSibling;
		return {
			node: h,
			offset: i
		}
	...
	function Za(a) {
		for (var b = document.getElementsByTagName("head")[0] || (O ? document.body : document.documentElement), c = document.createElement("link"), d = Db(ia(a, "absolute")), e = f('link[rel="stylesheet"]', b), g = 0, h = e.length; g < h; g++) if (Db(ia(e[g].href, "absolute")) === d) return;
		b.appendChild(c);
		c.href = a;
		c.rel = "stylesheet"
	}
	...
	function Zb() {
		...
		if (!(l && d !== "br") && (!ea || !(z < 3 && d !== "p")) && !(La && z < 9)) {
			...
			f(c).keyup(function(e) {
				...
				} else if (b.cmd.selection(), e = a(b.cmd.range), !(e == "marquee" || e == "select")) if (g[e] || P(c, "formatblock", "<p>"), e = b.cmd.commonAncestor("div")) {
	...
	if (!A.KindEditor) {
		...
		E(D, {
			init: function(a) {
				for (var a = Y(a) ? a : [a], b = 0, c = 0, d = a.length; c < d; c++) a[c] && (this[c] = a[c].constructor === D ? a[c][0] : a[c], b++);
				this.length = b;
				this.doc = T(this[0]);
				this.name = Ka(this[0]);
			...
			pos: function() {
				var a = this[0],
					b = 0,
					c = 0;
				if (a) if (a.getBoundingClientRect) a = a.getBoundingClientRect(), c = ba(this.doc), b = a.left + c.x, c = a.top + c.y;
		...
		E(na, {
			...
			select: function(a) {
				...
				if (a && d.nodeType == 1 && c.collapsed) {
					...
					if (W && (a = d.childNodes, f(d).isInline() || e > 0 && f(a[e - 1]).isInline() || a[e] && f(a[e]).isInline())) c.insertNode(g.createTextNode("\u200b")), k = !0
				}
				if (l) try {
					i = c.get(!0), i.select()
				} catch (r) {} else k && c.collapse(!1), i = c.get(!0), b.removeAllRanges(), b.addRange(i), g !== document && (c = f(i.endContainer).pos(), h.scrollTo(c.x, c.y));
				h.focus();
				return this
			},
			wrap: function(a) {
					...
					if (!g && a == e.start) g = !0;
						...
						if (!d && b.type == 3 && B(a.nodeValue).length > 0) {
							...
							if (b.type == 3) Ra(n).append(b.clone(!1)), b.replaceWith(n);
								...
								for (var a = b, j;
								(j = b.first()) && j.children().length == 1;) b = j;
								j = b.first();
								for (b = b.doc.createDocumentFragment(); j;) b.appendChild(j[0]), j = j.next();
								j = a.clone(!0);
								d = Ra(j);
								for (var p = j, l = !1; n;) {
			...
			remove: function(a) {
				...
				if (d > 0) {
					...
					j && da(j) && (j.remove(), c.setStart(b, d - 1), b == e && c.setEnd(e, n - 1));
			...
			val: function(a) {
				...
				if (a === "forecolor") return (a = this.commonNode({
					"*": ".color"
				})) && (c = a.css("color")), c = va(c), c === "" && (c = "default"), c.toLowerCase();
				if (a === "hilitecolor") return (a = this.commonNode({
					"*": ".background-color"
				})) && (c = a.css("background-color")), c = va(c), c === "" && (c = "default"), c.toLowerCase();
				return c
			},
			toggle: function(a, b) {
		...
		m("formatblock,selectall,justifyleft,justifycenter,justifyright,justifyfull,insertorderedlist,insertunorderedlist,indent,outdent,subscript,superscript".split(","), function(a, b) {
			na.prototype[b] = function(a) {
				this.select();
				P(this.doc, b, a);
				(!l || N(b, "formatblock,selectall,insertorderedlist,insertunorderedlist".split(",")) >= 0) && this.selection();
				return this
			}
		});
		...
		ta.prototype = {
			...
			create: function() {
				...
				}, f(A).bind("resize", b._fullscreenResizeHandler), h.select("fullscreen"), k.first().css("visibility", "hidden"), k.last().css("visibility", "hidden")) : (ea && f(A).bind("scroll", function() {
					b._scrollPos = ba()
				}), b.resizeType > 0 ? Sa({
					moveEl: g,
					clickEl: k,
			...
			text: function(a) {
				return a === o ? B(this.html().replace(/<(?!img|embed).*?>/ig, "").replace(/&nbsp;/ig, " ")) : this.html(C(a))
			},
			isEmpty: function() {
		...
		Eb("core", function(a) {
			...
			b.afterCreate(function() {
				function c() {
					...
					var d = j[0].innerHTML;
					j.remove();
					d !== "" && (W && (d = d.replace(/(<br>)\1/ig, "$1")), b.pasteType === 2 && (d = d.replace(/(<(?:p|p\s[^>]*)>) *(<\/p>)/ig, ""), /schemas-microsoft-com|worddocument|mso-\w+/i.test(d) ? d = lb(d, b.filterMode ? b.htmlTags : a.options.htmlTags) : (d = S(d, b.filterMode ? b.htmlTags : null), d = b.beforeSetHtml(d))), b.pasteType === 1 && (d = d.replace(/&nbsp;/ig, " "), d = d.replace(/\n\s*\n/g, "\n"), d = d.replace(/<br[^>]*>/ig, "\n"), d = d.replace(/<\/p><p[^>]*>/ig, "\n"), d = d.replace(/<[^>]+>/g, ""), d = d.replace(/ {2}/g, " &nbsp;"), b.newlineTag == "p" ? /\n/.test(d) && (d = d.replace(/^/, "<p>").replace(/$/, "<br /></p>").replace(/\n/g, "<br /></p><p>")) : d = d.replace(/\n/g, "<br />$&")), b.insertHtml(d, !0))
				}
				var d = b.edit.doc,
					f, i, j, n = "__kindeditor_paste__",
					m = !1;
				a(d.body).bind("paste", function(o) {
			...
			b.beforeSetHtml(function(a) {
				l && z <= 8 && (a = a.replace(/<input[^>]*>|<(select|button)[^>]*>[\s\S]*?<\/\1>/ig, function(a) {
					var b = H(a);
					if (aa(b.style || "").display == "none") return '<div class="ke-display-none" data-ke-input-tag="' + escape(a) + '"></div>';
					return a
				}));
