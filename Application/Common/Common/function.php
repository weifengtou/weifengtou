 ...
 * @author wodrow 
 */
function yc_tpGetLastId()
{
    // $_last_id = "SELECT   LAST_INSERT_ID()";
    $_last_id = "SELECT   @@IDENTITY";
    $tmp = M()-> query($_last_id);
    return $tmp[0]['@@IDENTITY'];
}
/**
 * 检测用户是否登录
...
*/
function sendMail($to, $subject, $content,$username) {
...
/**
 * 根据子id获取用户id
 * @param  int $child_id 用户id
 * @param  int $style 用户类型 1:项目人 2:投资人
 * @return int    用户id
 */
function get_homeuser_id($child_id,$style=0)
{
	if ($style==1):
		$info = M('Pruser')->where("id='$child_id'")->find();
	elseif($style==2):
		$info = M('Investor')->where("id='$child_id'")->find();
	else:
		#
	endif;
	return $info[uid];
 ...
 * 获取用户详细信息
 * @param  int $id 用户id
 * @return array()     用户详细(二维数组)
 */
function get_homeuser_detail($id)
{
    $find_style = M('Homeuser')->where("id='$id'")->field("style")->find();
    switch ($find_style[style]) {
        case '1':
            $sql = "SELECT
                __HOMEUSER__.id AS user_id,
                __HOMEUSER__.style,
                __HOMEUSER__.email,
                __HOMEUSER__.username,
                __HOMEUSER__.reg_time,
                __HOMEUSER__.update_time,
                __HOMEUSER__.`status`,
                CASE __PRUSER__.face_img 
                    WHEN '' THEN NULL
                    WHEN NULL THEN NULL
                ELSE  CONCAT('./Uploads/Album/',__PRUSER__.face_img) END AS face_img,
                __PRUSER__.realname,
                __PRUSER__.phone,
                __PRUSER__.sex,
                __PRUSER__.sheng,
                ...
                FROM
                __HOMEUSER__
                LEFT JOIN __PRUSER__ ON __HOMEUSER__.id = __PRUSER__.uid 
                WHERE __HOMEUSER__.id='$id'";
            break;
        case '2':
            $sql = "SELECT
                __HOMEUSER__.id AS user_id,
                ...
                __HOMEUSER__.update_time,
                __HOMEUSER__.`status`,
                CASE __INVESTOR__.face_img 
                    WHEN '' THEN NULL
                    WHEN NULL THEN NULL
                ELSE  CONCAT('./Uploads/Investor/',__INVESTOR__.face_img) END AS face_img,
                __INVESTOR__.type,
                __INVESTOR__.iden_status,
                __INVESTOR__.level_id,
                __INVESTOR__.privacy,
                __INVESTOR__.phone,
                __INVESTOR__.realname,
                __INVESTOR__.sex,
                __INVESTOR__.company,
                __INVESTOR__.postion,
                __INVESTOR__.intention,
                __INVESTOR__.intentionname,
                __INVESTOR__.tzidea,
                __INVESTOR__.income,
                __INVESTOR__.min_maney,
                __INVESTOR__.max_maney,
                __INVESTOR__.inv_info,
                __INVESTOR__.f_image,
                __INVESTOR__.b_image,
                __INVESTOR__.add_time,
                __INVESTOR__.update_time,
                __INVESTOR__.qq,
                __INVESTOR__.weibo,
                __INVESTOR__.sheng,
                __INVESTOR__.shengid,
                __INVESTOR__.shi,
                __INVESTOR__.shiid,
                __INVESTOR__.qu,
                __INVESTOR__.quid
                FROM
                __HOMEUSER__
                LEFT JOIN __INVESTOR__ ON __HOMEUSER__.id = __INVESTOR__.uid 
                WHERE __HOMEUSER__.id='$id'";
            break;
        case '3':
            $sql = "SELECT * FROM __HOMEUSER__ WHERE id='$id'";
            break;

        default:
            $detail = M('Homeuser')->where("id='$id'")->field("style")->select();
            break;
    }    
    return $detail?$detail:M()->query($sql);
}
...
/**
 * 客服提醒
 * @param  string $kefu_email 客服邮箱
 * @param  int $type       项目 投资人？
 * @param  array $data       提交信息
 * @return void             发送邮箱提醒
 */
function submit_remind($kefu_email,$type,$data)
{
    if ($type==1) {
        $content = "有新项目<span>".$data[pjname]."</span>提交,请及时处理!";
    }
    if ($type==2) {
        $content = "有新投资人<span>".$data[realname]."</span>提交,请及时处理!";
    }
    if ((sendMail($kefu_email,'微风投客服提醒',$content,'微风投客服'))==1) {
        // return true;
    }else{
        // return false;
    }
